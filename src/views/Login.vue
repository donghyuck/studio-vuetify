        ...
        <div class="d-flex flex-column fill-height justify-center pb-5">
            <v-sheet width="400" class="mx-auto pt-10 px-10 pb-10" rounded="xl" elevation="12" color="grey-lighten-3">
                ...
                <v-form validate-on="submit" @submit.prevent="doLogin">
                    ...
                    <v-text-field v-model="state.password" :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
                        :type="show ? 'text' : 'password'" @click:append="show = !show" label="Password"
                        variant="underlined" :error-messages="v$.password.$errors.map(e => e.$message)" required
                        clearable prepend-icon="mdi-lock" @input="v$.password.$touch" @blur="v$.password.$touch" />

                    <v-btn type="submit" block variant="tonal" class="mt-2">Login</v-btn>
                </v-form>
                <v-overlay v-model="overlay" close-on-back="false" contained class="align-center justify-center">
